...
(function (factory) {
    ...
    lang: {
      'en-US': {
        ...
        image: {
          ...
          dragImageHere: 'Drag an img here',
  ...
  var Popover = function () {
    ...
    this.update = function ($popover, oStyle) {
      ...
      var $imagePopover = $popover.find('.note-img-popover');
      if (oStyle.image) {
    ...
    this.update = function ($handle, oStyle) {
      ...
      if (oStyle.image) {
        ...
        $selection.css({
          ...
          width: szImage.w,
          height: szImage.h
        }).data('target', oStyle.image); // save current img element.
  ...
  var Dialog = function () {
    ...
    this.showImageDialog = function ($editable, $dialog) {
      return $.Deferred(function (deferred) {
        var $imageDialog = $dialog.find('.note-img-dialog');

        var $imageInput = $dialog.find('.note-img-input'),
            $imageUrl = $dialog.find('.note-img-url'),
            $imageBtn = $dialog.find('.note-img-btn');
  ...
  var Renderer = function () {
    ...
    tplPopover = function (lang) {
      return '<div class="note-popover">' +
                '<div class="note-link-popover popover bottom in" style="display: none;">' +
                  ...
                  '</div>' +
                '</div>' +
                '<div class="note-img-popover popover bottom in" style="display: none;">' +
                  '<div class="arrow"></div>' +
                  '<div class="popover-content note-img-content">' +
                    '<div class="btn-group">' +
    ...
    tplDialog = function (lang, options) {
      var tplImageDialog = function () {
        return '<div class="note-img-dialog modal" aria-hidden="false">' +
                   ...
                   '<div class="modal-content">' +
                     ...
                     '<div class="modal-body">' +
                       '<div class="row-fluid">' +
                         ...
                         '<input class="note-img-input" type="file" name="files" accept="img/*" />' +
                         '<h5>' + lang.image.url + '</h5>' +
                         '<input class="note-img-url form-control span12" type="text" />' +
                       '</div>' +
                     '</div>' +
                     '<div class="modal-footer">' +
                       '<button href="#" class="btn btn-primary note-img-btn disabled" disabled="disabled">' + lang.image.insert + '</button>' +
                     '</div>' +
                   '</div>' +
                 '</div>' +
               '</div>';
