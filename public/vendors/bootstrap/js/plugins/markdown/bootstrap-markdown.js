    ...
    buttons: [
      [{
        ...
        data: [{
          ...
          callback: function(e){
            // Give ![] surround the selection and prepend the img link
            var chunk, cursor, selected = e.getSelection(), content = e.getContent(), link

            if (selected.length == 0) {
              // Give extra word
              chunk = 'enter img description here'
            } else {
              chunk = selected.text
            }
            ...
            if (link != null) {
              // transform selection and set the cursor into chunked text
              e.replaceSelection('!['+chunk+']('+link+' "enter img title here")')
              cursor = selected.start+2

              // Set the next tab
              e.setNextTab('enter img title here')
